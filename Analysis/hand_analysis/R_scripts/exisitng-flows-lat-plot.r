...
netfilter <- read.table("/home/rishabh/Analysis/hand_analysis/prefstats/nopref/_home_necto_vnds_nf_vignat-mg-existing-flows-latency.results")
netfilter["middlebox"] <- "No_Prefetching"
vignat <- read.table("/home/rishabh/Analysis/hand_analysis/prefstats/pref/_home_necto_vnds_nf_vignat-mg-existing-flows-latency.results")
...
p <- ggplot(data_summary, aes(x=V1/1e3, y=V4/1e3,
     ...
     geom_errorbar(aes(ymin=(V4-ci)/1e3, ymax=(V4+ci)/1e3), width=.01,
                   position=pd) +
     labs(title="Latency for ~100Kpkt/s. 500 existing probe flows/s") +
     xlab("# concurrent flows (K)") +
     ylab(bquote("1-way latency, only existing flows, "*mu*"s")) +
     theme_bw() +
...
ggsave(filename="latency-existing-flows.png", width=8, height=4)
print(p)
